...
//
//! `arith` dialect handlers — full port of `ktir_cpu/dialects/arith_ops.py`
//! (which dispatches into `ktir_cpu/ops/arith_ops.py::ArithOps`).
//!
    ...
    match cond {
        Value::Tile(c) => {
            ...
            let data: Vec<f32> = (0..n)
                .map(|i| if c.data[i] != 0.0 { t[i] } else { f[i] })
                .collect();
...
fn sitofp(op: &Operation, ctx: &mut CoreContext, _env: &ExecutionEnv) -> Result<Option<Value>, String> {
    ...
    match v {
        Value::Tile(t) => {
            let data: Vec<f32> = t.data.iter().map(|&x| round_i64(x) as f32).collect();
...
fn fptosi(op: &Operation, ctx: &mut CoreContext, _env: &ExecutionEnv) -> Result<Option<Value>, String> {
    ...
    match v {
        Value::Tile(t) => {
            let data: Vec<f32> = t.data.iter().map(|&x| x.trunc()).collect();
...
fn unary_float(
    ...
    match v {
        ...
        Value::Tile(t) => {
            let data: Vec<f32> = t.data.iter().map(|&x| f(x)).collect();
...
fn compare(
    ...
    // Tile path — broadcast a scalar across the tile's shape.
    let (shape, n) = match (a, b) {
        (Value::Tile(t), _) | (_, Value::Tile(t)) => (t.shape.clone(), t.data.len()),
        _ => unreachable!(),
    };
    let lhs = elementwise_data(a, n, name)?;
    let rhs = elementwise_data(b, n, name)?;
    let data: Vec<f32> = (0..n)
        .map(|i| if cmp(lhs[i], rhs[i]) { 1.0 } else { 0.0 })
        .collect();
...
/// bit pattern. No `half` dependency yet; used by truncf / convertf-to-f16.
fn narrow_f16(x: f32) -> u16 {
    ...
    if exp <= 0 {
        // Subnormal or underflow -> flush small magnitudes to signed zero.
        if exp < -10 {
            return sign;
        }
        ...
        // Round to nearest, ties to even, on the dropped low 13 bits. The round
        // bit is 0x1000; sticky bits are the low 12. A tie (round set, sticky
        // clear) only rounds up when the kept mantissa LSB is already odd.
