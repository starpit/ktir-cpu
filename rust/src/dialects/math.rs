...
//
//! `math` dialect handlers — port of `ktir_cpu/dialects/math_ops.py` and the
//! `MathOps` compute helpers in `ktir_cpu/ops/math_ops.py`.
//!
...
/// in `ktir_cpu/dialects/math_ops.py`.
...
fn unary(
    ...
    match v {
        Value::Tile(t) => {
            ...
            let data: Vec<f32> = t.data.iter().map(|&x| round_to(f(x), dtype)).collect();
...
mod tests {
    ...
    fn exp_tile_elementwise() {
        ...
        let t = as_tile(&r);
        close(t.data[0], 1.0);
        close(t.data[1], std::f32::consts::E);
        close(t.data[2], (2.0f32).exp());
    }
    ...
    fn erf_matches_known_values() {
        ...
        close(t.data[0], 0.0);
        // erf(1) ≈ 0.8427007
        close(t.data[1], 0.8427007);
        // erf is odd: erf(-1) = -erf(1)
        close(t.data[2], -0.8427007);
    }
    ...
    fn fma_tile() {
        ...
        let t = as_tile(&r);
        close(t.data[0], 9.0); // 2*4 + 1
        close(t.data[1], 16.0); // 3*5 + 1
    }
