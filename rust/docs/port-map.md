...
- **MemRef**: base_ptr: int, shape: Tuple[int, ...], strides: List[int], memory_space: str, dtype: str, coordinate_set: Optional[str], lx_core_id: Optional[int].
...
Now I have all the information needed to create a comprehensive port specification. Let me compile the dense markdown spec:
...
- `dim` (int, optional): Axis to reduce. `None` → flatten entire tensor to scalar.
...
**Semantics:**  
Folds the input tensor along `dim` using a **pairwise tree reduction** of the combiner region. MLIR requires the combiner to be associative, so fold order is free. Tree reduction executes `ceil(log2(N))` vectorized region calls instead of N sequential steps. Each region call combines whole sub-array slices as Tiles.

- If `dim = None`: flatten to 1-D, reduce to scalar (shape `()`).
- If `dim = k`: squeeze dimension k, result has shape with axis k removed.

Odd-length slices carry forward to the next round (line 127–129: `if n % 2: combined = np.concatenate([combined, tail], axis=dim)`).

**Execution path:**  
`_tree_fold()` (line 86–133): recursively splits `acc` in half along `dim`, calls `_run_combiner()` on slices as Tiles, concatenates result with unpaired tail, repeats until `n=1`.

`_run_combiner()` (line 61–83): pushes scope, binds `bb0_names[0]` ← `lhs`, `bb0_names[1]` ← `rhs`, executes region via `env.execute_region()`, pops scope, unwraps `_YieldResult`.

**Output binding:**  
Both `result` name and `outs_var` (if present) are bound to context (line 216–217); allows downstream ops to reference by either name.
