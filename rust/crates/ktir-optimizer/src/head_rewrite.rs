...
pub fn recognize_head_attention(func: &IRFunction) -> Option<HeadAttnIsland> {
    ...
    for op in &func.operations {
        match op.op_type.as_str() {
            ...
            "arith.constant" => {
                if let (Some(res), Some(Attr::Int(v))) =
                    (&op.result, op.attributes.get("value"))
                {
...
fn trace_divf<'a>(
    w: &str,
    def: &'a HashMap<String, &'a Operation>,
...
pub fn rewrite_head_attention(isl: &HeadAttnIsland) -> IRFunction {
    ...
    ops.push(Operation::new(Some(&sc_scaled), "arith.mulf", &[&sc_raw, &sc_scl]));
    ...
    let sc = g.next("sc");
    ops.push(Operation::new(Some(&sc), "arith.addf", &[&sc_scaled, &mask_b]));
...
mod tests {
    ...
    fn rewrite_emits_no_control_flow_and_one_store() {
        ...
        let stores = f.operations.iter().filter(|o| o.op_type == "ktdp.store").count();
        ...
        let mms = f.operations.iter().filter(|o| o.op_type == "linalg.matmul").count();
        assert_eq!(mms, 4, "two QKᵀ + two AV, once");
        // Preserves the per-head selection arithmetic as SSA.
        assert!(f.operations.iter().any(|o| o.op_type == "ktdp.get_compute_tile_id"));
        assert!(f.operations.iter().any(|o| o.op_type == "arith.divui"));
        let muls = f.operations.iter().filter(|o| o.op_type == "arith.muli").count();
