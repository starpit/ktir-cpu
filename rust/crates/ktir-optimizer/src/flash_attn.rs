...
pub fn apply_flash_attention(
    ...
    for name in names {
        let Some(func) = module.functions.get(&name) else { continue };
        ...
        if let Some(island) = recognize_rerolled_attention(func) {
            ...
            let blk = choose_block_budgeted(island.m, island.cap, dtype_bytes(&island.dtype), &needs_flash);
        ...
        let Some(island) = recognize_attention(func) else { continue };
        if !needs_flash(island.scores_bytes()) {
...
pub fn recognize_attention(func: &IRFunction) -> Option<AttentionIsland> {
    ...
    if func
        .operations
        .iter()
        .any(|op| matches!(op.op_type.as_str(), "scf.for" | "scf.if" | "scf.while" | "scf.parallel" | "scf.forall"))
    {
        return None;
    }
        ...
        match op.op_type.as_str() {
            "ktdp.construct_memory_view" => {
                if let (Some(res), Some(arg)) = (&op.result, op.operands.first()) {
                    views.insert(
                        res.clone(),
                        ViewInfo { arg: arg.clone(), shape: shape_attr(op), dtype: dtype_attr(op) },
                    );
                }
            }
            ...
            "ktdp.load" => {
                ...
                {
                    load_src.insert(res.clone(), (vi.arg.clone(), vi.shape.clone(), vi.dtype.clone()));
                }
            }
            _ => {}
        }
    ...
    let stores: Vec<&Operation> =
        func.operations.iter().filter(|o| o.op_type == "ktdp.store").collect();
...
fn broadcast_traces_to_reduce(
    ...
    let Some(bop) = def.get(bcast_ssa) else { return false };
    if bop.op_type != "linalg.broadcast" {
        return false;
    }
    let Some(src) = bop.operands.first() else { return false };
...
fn traces_to_reduce_of(
    ...
    for _ in 0..4 {
        let Some(op) = def.get(&cur) else { return false };
        if matches!(op.op_type.as_str(), "tensor.reshape" | "tensor.extract" | "tensor.expand_shape" | "tensor.collapse_shape")
        {
...
fn choose_block_budgeted(
    ...
    let footprint =
        |b: i64| (m as usize).saturating_mul(b as usize).saturating_mul(bytes);
...
impl NameGen {
    fn new(prefix: &str) -> Self {
        NameGen { n: 0, prefix: prefix.to_string() }
    }
...
pub fn tile_attention(island: &AttentionIsland) -> IRFunction {
    ...
    let raw = g.next("raw");
    body.push(Operation::new(Some(&raw), "linalg.matmul", &[&q, &kjt, &raw_init]));

    // scaled = raw * scale_splat
    let scale_t = g.next("sct");
    body.push(mk_splat(&scale_t, &c_scale, &[isl.m, bc], dt));
    let scaled = g.next("scaled");
    body.push(Operation::new(Some(&scaled), "arith.mulf", &[&raw, &scale_t]));
    ...
    let sj = if isl.causal {
        ...
        body.push(Operation::new(Some(&masked), "arith.addf", &[&scaled, &mask]));
        masked
    } else {
        scaled
    };
    ...
    body.push(Operation::new(Some(&mnew), "arith.maximumf", &[&mi, &rmax2]));

    // m_new broadcast to [m, BC]
    let mnew_b = broadcast_col_to(&mut g, &mut body, &mnew, isl.m, bc, dt);
    // shifted = sj - m_new_b
    let shifted = g.next("shift");
    body.push(Operation::new(Some(&shifted), "arith.subf", &[&sj, &mnew_b]));
    // P = exp(shifted) -> [m, BC]
    let p = g.next("p");
    ...
    body.push(Operation::new(Some(&acc_scaled), "arith.mulf", &[&alpha_b, &acci]));
    let pv_init = g.next("pvi");
    body.push(mk_empty(&pv_init, &[isl.m, isl.d], dt));
    let pv = g.next("pv");
    body.push(Operation::new(Some(&pv), "linalg.matmul", &[&p, &vj, &pv_init]));
    let accnew = g.next("accnew");
    body.push(Operation::new(Some(&accnew), "arith.addf", &[&acc_scaled, &pv]));
    ...
    let mut forop = Operation::new(None, "scf.for", &[&lb, &ub, &step, &m0, &l0, &acc0])
        ...
        .with_attr("iter_args", Attr::StrList(vec![mi.clone(), li.clone(), acci.clone()]))
        .with_attr(
            "result_names",
...
fn causal_mask_block(
    ...
    ops.push(Operation::new(Some(&mask), "arith.select", &[&cond, &zero_t, &ninf_t]));
    mask
}
...
pub fn recognize_rerolled_attention(func: &IRFunction) -> Option<ReRolledIsland> {
    ...
    if !func.operations.iter().any(|o| o.op_type == "ktdp.get_compute_tile_id") {
        return None;
    }
    let gqac = func
        .operations
...
fn rr_walk_block(
    ...
    Some(RrBlock { v, q, k, scale, masked_scores, probs, row_sum })
}
...
fn rr_is_context(blk: &RrBlock, def: &HashMap<String, &Operation>) -> bool {
    let Some(sop) = def.get(&blk.masked_scores) else { return false };
    let Some(mask) = sop.operands.get(1) else { return false };
    let Some(mop) = def.get(mask) else { return false };
...
fn rr_peel_broadcast_reshape(ssa: &str, def: &HashMap<String, &Operation>) -> Option<String> {
    ...
    let sop = def.get(src)?;
    if matches!(sop.op_type.as_str(), "tensor.reshape" | "tensor.expand_shape") {
...
fn rr_gm_is_max_of_reduces(
    ...
    let Some(a) = mop.operands.first() else { return false };
    let Some(b) = mop.operands.get(1) else { return false };
...
fn rr_reduce_of(
    ...
    for _ in 0..3 {
        let op = def.get(&cur)?;
        if matches!(op.op_type.as_str(), "tensor.reshape" | "tensor.expand_shape" | "tensor.collapse_shape")
        {
...
fn rr_check_shared_gs(
    ...
    let ok = (a == &ctx.row_sum && b == &diag.row_sum)
        || (a == &diag.row_sum && b == &ctx.row_sum);
...
fn rr_recover_tri_ninf(diag: &RrBlock, def: &HashMap<String, &Operation>) -> Option<f32> {
    ...
    match top.attributes.get("value") {
        Some(Attr::FloatList(v)) => v.iter().cloned().fold(None, |acc, x| match acc {
...
pub fn tile_rerolled_attention(isl: &ReRolledIsland, blk: i64) -> IRFunction {
    ...
    let blk = if blk >= 1 && cap % blk == 0 { blk } else { choose_block(cap) };
    let nb = cap / blk;
    let mut g = NameGen::new("fa");
    let mut ops: Vec<Operation> = Vec::new();
    ...
    body.push(Operation::new(Some(&sj_scaled), "arith.mulf", &[&sj_raw, &sjscl]));
    ...
    let sj = g.next("sj");
    body.push(Operation::new(Some(&sj), "arith.addf", &[&sj_scaled, &maskj_b]));
    ...
    ops.push(Operation::new(Some(&sd_scaled), "arith.mulf", &[&sd_raw, &sd_scl]));
    let tri = g.next("tri");
    ...
    ops.push(Operation::new(Some(&acc_rb), "arith.mulf", &[&cfac_bd, &acc_f]));
    let lc_rb = g.next("lcrb");
    ops.push(Operation::new(Some(&lc_rb), "arith.mulf", &[&cfac, &lc_row]));
    ...
    let o_at = g.next("oat");
    ops.push(
        Operation::new(Some(&o_at), "ktdp.construct_access_tile", &[&o_view, &c0, &qcol])
...
mod tests {
    ...
    fn rejects_non_attention() {
        ...
        let f = IRFunction {
            name: "copy".into(),
            arguments: vec![("%in".into(), "index".into()), ("%out".into(), "index".into())],
            grid: (1, 1, 1),
            return_type: None,
            operations: vec![
        ...
        assert!(recognize_attention(&f).is_none(), "copy node must not be recognized");
    }
    ...
    fn rejects_region_bearing() {
        ...
        assert!(recognize_attention(&f).is_none(), "region-bearing func bails");
    }
    ...
    fn tile_emits_scf_for_and_no_insert_slice() {
        ...
        let fors: Vec<&Operation> =
            tiled.operations.iter().filter(|o| o.op_type == "scf.for").collect();
        ...
        let block_tiles = body.iter().filter(|o| o.op_type == "ktdp.construct_access_tile").count();
        ...
        let slices = body.iter().filter(|o| o.op_type == "tensor.extract_slice").count();
        assert_eq!(slices, 0, "KV blocks read via ktdp access tile, not extract_slice");
    ...
    fn tile_preserves_args_and_grid() {
        ...
        assert_eq!(tiled.grid, (1, 1, 1), "rewritten node runs single-grid generic");
    }
    ...
    use crate::head_rewrite::{rewrite_head_attention, HeadAttnIsland};
    ...
    fn tile_rerolled_emits_one_scf_for_no_insert_slice() {
        ...
        fn has_bad(ops: &[Operation]) -> bool {
            ops.iter().any(|o| {
                matches!(o.op_type.as_str(), "tensor.insert_slice" | "tensor.extract_slice")
                    || o.regions.iter().any(|r| has_bad(r))
            })
        }
        assert!(!has_bad(&tiled.operations), "must not emit insert/extract_slice");

        // Per-head selection arithmetic preserved.
        assert!(tiled.operations.iter().any(|o| o.op_type == "ktdp.get_compute_tile_id"));
    ...
    fn rerolled_per_block_tile_fits_budget_and_shrinks() {
        ...
        assert!(needs(isl.scores_bytes()), "full tile must overflow at this budget");
        ...
        assert!(!needs(per_block), "per-block tile {per_block} must fit budget");
        ...
        let tiled = tile_rerolled_attention(&isl, blk);
        let forop = tiled.operations.iter().find(|o| o.op_type == "scf.for").unwrap();
        for op in &forop.regions[0] {
                ...
                && s[0] == isl.m
            {
                ...
                assert!(s[1] <= blk, "loop tile [{},{}] exceeds blk {blk}", s[0], s[1]);
            }
        }
    }
}
...
pub mod test_support {
    ...
    pub fn naive_attention(m: i64, cap: i64, d: i64, scale: f32, causal: bool) -> IRFunction {
        ...
        let raw = g.next("raw");
        ops.push(Operation::new(Some(&raw), "linalg.matmul", &[&q, &kt, &rawi]));
        // scaled = raw * scale
        let sc = g.next("sc");
        ...
        let sm = if causal {
            ...
            ops.push(Operation::new(Some(&masked), "arith.addf", &[&scaled, &mask]));
            masked
        } else {
            scaled
        };
    ...
    fn causal_mask_full(res: &str, m: i64, cap: i64, dt: &str) -> Operation {
        ...
        let cmp = Operation::new(Some("%fm_vis"), "arith.cmpi", &["%kc", "%fm_aq"])
            .with_attr("predicate", Attr::Str("sle".into()));
        let zero = Operation::new(Some("%fm_zero"), "arith.constant", &[]).with_attr("value", Attr::Float(0.0));
        let ninf = Operation::new(Some("%fm_ninf"), "arith.constant", &[])
            .with_attr("value", Attr::Float(-1.0e30));
        let sel = Operation::new(Some("%fm_v"), "arith.select", &["%fm_vis", "%fm_zero", "%fm_ninf"]);
