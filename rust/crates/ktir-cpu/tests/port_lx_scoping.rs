...
//! scratchpad lifetime accounting (RFC 0682, `ktir_cpu/grid.py`).
//!
...
//!   methods on `ktir_cpu::context::CoreContext`, so each case ports 1:1.
...
use ktir_cpu::memory::{HBMSimulator, LXScratchpad, STICK_BYTES};
...
fn make_context(lx_size_mb: i64) -> CoreContext {
    let lx = Rc::new(RefCell::new(LXScratchpad::new(0, lx_size_mb)));
    let hbm = Rc::new(RefCell::new(HBMSimulator::default()));
