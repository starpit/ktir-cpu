...
uv run --no-project --with "transformers>=4.45" --with torch --with numpy --with safetensors \
  python rust/crates/ktir-cpu/tests/fixtures/gen_golden.py smollm2-135m smollm2-135m-prefill
```
...
```bash
cargo test -p ktir-cpu --features metal --test e2e_real_forward -- --test-threads=1
```
Prefill (m>1) is `#[cfg(feature = "metal")]` (the fused [1,1] segments need the
GPU/AMX offload for full-M reconstruction). Decode runs in default `cargo test`.
