module {
  func.func @ktir_decode_smollm2_135m_n372(%t705_ptr: index, %t706_ptr: index, %t707_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t705_ptr, sizes: [1, 1536], strides: [1536, 1] {
    ...
    %view3 = ktdp.construct_memory_view %t706_ptr, sizes: [1, 1536], strides: [1536, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 1535 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x1536xf16>
    ...
    %view13 = ktdp.construct_memory_view %t707_ptr, sizes: [1, 1536], strides: [1536, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 1535 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x1536xf16>
