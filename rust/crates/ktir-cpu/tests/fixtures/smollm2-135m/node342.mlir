module {
  func.func @ktir_decode_smollm2_135m_n342(%t675_ptr: index, %t676_ptr: index, %t677_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t675_ptr, sizes: [1, 1536], strides: [1536, 1] {
    ...
    %view3 = ktdp.construct_memory_view %t676_ptr, sizes: [1, 1536], strides: [1536, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 1535 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x1536xf16>
    ...
    %view13 = ktdp.construct_memory_view %t677_ptr, sizes: [1, 1536], strides: [1536, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 1535 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x1536xf16>
