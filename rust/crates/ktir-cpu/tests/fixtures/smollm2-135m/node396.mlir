module {
  func.func @ktir_decode_smollm2_135m_n396(%t729_ptr: index, %t731_ptr: index, %t787_ptr: index, %t293_ptr: index, %t730_ptr: index, %t294_ptr: index, %t728_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t729_ptr, sizes: [1, 576], strides: [576, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 575 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x576xf16>
    %view1 = ktdp.construct_memory_view %t731_ptr, sizes: [1, 576], strides: [576, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 575 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x576xf16>
    ...
    %view6 = ktdp.construct_memory_view %t730_ptr, sizes: [1, 192], strides: [192, 1] {
    ...
    %view8 = ktdp.construct_memory_view %t728_ptr, sizes: [1, 192], strides: [192, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 191 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x192xf16>
