module {
  func.func @ktir_decode_smollm2_135m_n376(%t710_ptr: index, %t279_ptr: index, %t711_ptr: index) attributes {grid = [1, 1]} {
    ...
    %view2 = ktdp.construct_memory_view %t711_ptr, sizes: [1, 576], strides: [576, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 575 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x576xf16>
