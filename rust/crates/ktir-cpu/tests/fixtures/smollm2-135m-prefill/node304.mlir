module {
  func.func @ktir_prefill_smollm2_135m_n304(%t636_ptr: index, %t639_ptr: index, %t5_ptr: index, %t6_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %half0 = arith.constant 32 : index
    %view1 = ktdp.construct_memory_view %t636_ptr, sizes: [288, 64], strides: [64, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 287 >= 0, d1 >= 0, -d1 + 63 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<288x64xf16>
    %view2 = ktdp.construct_memory_view %t639_ptr, sizes: [288, 64], strides: [64, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 287 >= 0, d1 >= 0, -d1 + 63 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<288x64xf16>
