module {
  func.func @ktir_prefill_smollm2_135m_n101(%t434_ptr: index, %t78_ptr: index, %t436_ptr: index) attributes {grid = [32, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t434_ptr, sizes: [32, 576], strides: [576, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 31 >= 0, d1 >= 0, -d1 + 575 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<32x576xf16>
    %view1 = ktdp.construct_memory_view %t78_ptr, sizes: [1536, 576], strides: [576, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 1535 >= 0, d1 >= 0, -d1 + 575 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1536x576xf16>
    %view2 = ktdp.construct_memory_view %t436_ptr, sizes: [32, 1536], strides: [1536, 1] {
