module {
  func.func @ktir_prefill_smollm2_135m_n193(%t527_ptr: index, %t145_ptr: index, %t528_ptr: index) attributes {grid = [32, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t527_ptr, sizes: [32, 1536], strides: [1536, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 31 >= 0, d1 >= 0, -d1 + 1535 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<32x1536xf16>
    %view1 = ktdp.construct_memory_view %t145_ptr, sizes: [576, 1536], strides: [1536, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 575 >= 0, d1 >= 0, -d1 + 1535 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<576x1536xf16>
    %view2 = ktdp.construct_memory_view %t528_ptr, sizes: [32, 576], strides: [576, 1] {
