module {
  func.func @ktir_decode_llama_3_2_1b_n186(%t365_ptr: index, %t367_ptr: index, %t423_ptr: index, %t139_ptr: index, %t366_ptr: index, %t140_ptr: index, %t364_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t365_ptr, sizes: [1, 2048], strides: [2048, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 2047 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x2048xf16>
    %view1 = ktdp.construct_memory_view %t367_ptr, sizes: [1, 2048], strides: [2048, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 2047 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x2048xf16>
    ...
    %view6 = ktdp.construct_memory_view %t366_ptr, sizes: [1, 512], strides: [512, 1] {
    ...
    %view8 = ktdp.construct_memory_view %t364_ptr, sizes: [1, 512], strides: [512, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 511 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x512xf16>
