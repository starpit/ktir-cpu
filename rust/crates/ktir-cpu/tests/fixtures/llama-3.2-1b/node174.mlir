module {
  func.func @ktir_decode_llama_3_2_1b_n174(%t354_ptr: index, %t131_ptr: index, %t355_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t354_ptr, sizes: [1, 2048], strides: [2048, 1] {
    ...
    %v2 = ktdp.load %acc1 : !ktdp.access_tile<1x2048xindex> -> tensor<1x2048xf16>
    %xf3 = arith.extf %v2 : tensor<1x2048xf16> to tensor<1x2048xf32>
    %x24 = arith.mulf %xf3, %xf3 : tensor<1x2048xf32>
    %zero5 = arith.constant 0.0 : f32
    %sinit6 = tensor.splat %zero5 : tensor<1xf32>
    %ssum7 = linalg.reduce { arith.addf }
      ins(%x24 : tensor<1x2048xf32>)
    ...
    %invinit19 = tensor.empty() : tensor<1x2048xf16>
    %invb20 = linalg.broadcast ins(%invE18 : tensor<1xf16>) outs(%invinit19 : tensor<1x2048xf16>) dimensions = [1]
    %xs21 = arith.mulf %v2, %invb20 : tensor<1x2048xf16>
    %view22 = ktdp.construct_memory_view %t131_ptr, sizes: [2048], strides: [1] {
    ...
    %ginit25 = tensor.empty() : tensor<1x2048xf16>
    %gb26 = linalg.broadcast ins(%v24 : tensor<2048xf16>) outs(%ginit25 : tensor<1x2048xf16>) dimensions = [0]
    %y27 = arith.mulf %xs21, %gb26 : tensor<1x2048xf16>
    %view28 = ktdp.construct_memory_view %t355_ptr, sizes: [1, 2048], strides: [2048, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 2047 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x2048xf16>
    ...
    ktdp.store %y27, %acc29 : tensor<1x2048xf16>, !ktdp.access_tile<1x2048xindex>
    return
  }
}
