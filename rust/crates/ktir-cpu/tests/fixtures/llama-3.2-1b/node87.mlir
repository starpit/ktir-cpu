module {
  func.func @ktir_decode_llama_3_2_1b_n87(%t266_ptr: index, %t267_ptr: index, %t268_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t266_ptr, sizes: [1, 8192], strides: [8192, 1] {
    ...
    %view3 = ktdp.construct_memory_view %t267_ptr, sizes: [1, 8192], strides: [8192, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 8191 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x8192xf16>
    ...
    %view13 = ktdp.construct_memory_view %t268_ptr, sizes: [1, 8192], strides: [8192, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 8191 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x8192xf16>
