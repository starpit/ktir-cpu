module {
  func.func @ktir_decode_llama_3_2_1b_n148(%t328_ptr: index, %t112_ptr: index, %t329_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t328_ptr, sizes: [1, 8192], strides: [8192, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 0 >= 0, d1 >= 0, -d1 + 8191 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<1x8192xf16>
    %view1 = ktdp.construct_memory_view %t112_ptr, sizes: [2048, 8192], strides: [8192, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 2047 >= 0, d1 >= 0, -d1 + 8191 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<2048x8192xf16>
    %view2 = ktdp.construct_memory_view %t329_ptr, sizes: [1, 2048], strides: [2048, 1] {
