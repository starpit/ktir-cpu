module {
  func.func @ktir_prefill_llama_3_2_1b_n117(%t296_ptr: index, %t297_ptr: index, %t298_ptr: index) attributes {grid = [1, 1]} {
    %c0 = arith.constant 0 : index
    %view0 = ktdp.construct_memory_view %t296_ptr, sizes: [32, 8192], strides: [8192, 1] {
    ...
    %view3 = ktdp.construct_memory_view %t297_ptr, sizes: [32, 8192], strides: [8192, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 31 >= 0, d1 >= 0, -d1 + 8191 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<32x8192xf16>
    ...
    %view13 = ktdp.construct_memory_view %t298_ptr, sizes: [32, 8192], strides: [8192, 1] {
      coordinate_set = affine_set<(d0, d1) : (d0 >= 0, -d0 + 31 >= 0, d1 >= 0, -d1 + 8191 >= 0)>,
      memory_space = #ktdp.spyre_memory_space<HBM>
    } : memref<32x8192xf16>
