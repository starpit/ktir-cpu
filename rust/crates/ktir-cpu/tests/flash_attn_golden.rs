...
use ktir_cpu::ir::{IRModule, Operation};
...
fn run_attn(
    ...
    let args: &[(&str, Arg)] = &[
        ("q_ptr", Arg::Tensor { data: q.to_vec(), shape: vec![m, d], dtype: DType::F16 }),
        ("k_ptr", Arg::Tensor { data: k.to_vec(), shape: vec![cap, d], dtype: DType::F16 }),
        ("v_ptr", Arg::Tensor { data: v.to_vec(), shape: vec![cap, d], dtype: DType::F16 }),
        ("o_ptr", Arg::Tensor { data: vec![0.0; m * d], shape: vec![m, d], dtype: DType::F16 }),
    ];
    let out = execute_function(&module, "attn", args).expect("attention run");
    let Output { data, .. } = out.get("o_ptr").or_else(|| out.get("%o_ptr")).expect("o_ptr output");
    data.clone()
}
...
fn reference_attention(q: &[f32], k: &[f32], v: &[f32], cfg: &AttnCfg) -> Vec<f32> {
    let AttnCfg { m, cap, d, scale, causal } = *cfg;
    ...
    for qr in 0..m {
        ...
        for (kc, pk) in p.iter_mut().enumerate() {
            let e = if s[kc] == f32::NEG_INFINITY { 0.0 } else { (s[kc] - mx).exp() };
            *pk = e;
            sum += e;
        }
...
fn tiled_matches_naive_causal() {
    ...
    let reference =
        reference_attention(&q, &k, &v, &AttnCfg { m, cap, d, scale, causal: true });
    ...
    assert!(diff_tiled_vs_naive < 1e-3, "tiled vs naive {diff_tiled_vs_naive} >= 1e-3");
    assert!(diff_tiled_vs_naive < 0.05, "tiled vs naive over golden gate");
    assert!(diff_tiled_vs_ref < 0.05, "tiled vs f32 reference over golden gate");
}
...
fn tiled_matches_naive_noncausal() {
    ...
    let tiled_out = run_attn(&tiled, &q, &k, &v, m, cap, d);
    let reference =
        reference_attention(&q, &k, &v, &AttnCfg { m, cap, d, scale, causal: false });
...
fn tiny_handcomputed_reference() {
    ...
    eprintln!("tiny: tiled={:?} naive={:?} expected={expected:.5}", tiled_out, naive_out);
    assert!((tiled_out[0] - expected).abs() < 0.02, "tiled {} vs {expected}", tiled_out[0]);
    assert!((naive_out[0] - expected).abs() < 0.02, "naive {} vs {expected}", naive_out[0]);
    assert!((tiled_out[0] - naive_out[0]).abs() < 1e-3, "tiled vs naive disagree");
}
...
fn negative_non_attention_is_unrecognized() {
    use ktir_cpu::ir::{Attr, IRFunction};
    ...
    assert!(recognize_attention(&f).is_none(), "copy node must not be recognized as attention");
}
...
fn real_cached_nodes_flash_after_head_rewrite() {
    ...
    for b in bundles {
        ...
        assert!(fired > 0, "{b}: flash_attn must FIRE on the re-rolled node111 (got {fired})");
...
fn forced_fire_through_program_execute() {
    ...
    assert_eq!(fired, 1, "FA must fire on the canonical node at a tiny budget");
    ...
    let spec = ProgramSpec {
        nodes: vec![NodeSpec {
            func: "attn_node".into(),
            bindings: vec![
                Binding { arg: "%t0_ptr".into(), tensor: 0, is_output: false },
                Binding { arg: "%t1_ptr".into(), tensor: 1, is_output: false },
                Binding { arg: "%t2_ptr".into(), tensor: 2, is_output: false },
                Binding { arg: "%t3_ptr".into(), tensor: 3, is_output: true },
            ],
        }],
    ...
    let args: &[(&str, Arg)] = &[
        ("t0", Arg::Tensor { data: q.clone(), shape: vec![m, d], dtype: DType::F16 }),
        ("t1", Arg::Tensor { data: k.clone(), shape: vec![cap, d], dtype: DType::F16 }),
        ("t2", Arg::Tensor { data: v.clone(), shape: vec![cap, d], dtype: DType::F16 }),
    ];
    let out = ktir_cpu::segmented::execute_segmented(&module, &spec, args, &["t3"])
...
#[cfg(feature = "metal")]
fn rename_arg(func: &mut ktir_cpu::ir::IRFunction, from: &str, to: &str) {
...
/// shows up).
#[cfg(feature = "metal")]
fn build_args(func: &ktir_cpu::ir::IRFunction) -> Vec<(String, Arg)> {
    ...
    for (arg_name, _) in &func.arguments {
        ...
        let data: Vec<f32> =
            (0..n).map(|i| (((i * 7 + seed) % 23) as f32 - 11.0) * 0.03).collect();
        args.push((
...
fn rerolled_flash_equals_head_rewrite(model: &str) {
    ...
    if ktir_cpu::metal_backend::NaxGemm::new().is_err() {
    ...
    let fname = module.functions.keys().next().expect("one function").clone();
    ...
    assert_eq!(hr, 1, "{model}: head_rewrite must fire (re-rolled reference)");
    ...
    let fired = apply_flash_attention(&mut flash_mod, |sb| attention_needs_flash(sb, FORCED_BUDGET));
    assert!(fired > 0, "{model}: flash_attn must FIRE on the re-rolled node (got {fired})");
    ...
    let forop = ffn.operations.iter().find(|o| o.op_type == "scf.for").expect("scf.for");
    ...
    assert!(blk < isl.cap, "{model}: must tile the cap axis (blk {blk} < cap {})", isl.cap);
    assert!(
        per_block < full,
    ...
    eprintln!("{model}/node111: flash-tiled vs head-rewritten over {compared} tensors, worst max-abs {worst:.6}");
