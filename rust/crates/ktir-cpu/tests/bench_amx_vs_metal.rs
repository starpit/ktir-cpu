...
//! (`metal_backend::NaxGemm::run`, f16) — at the per-kernel bench's logical GEMM
...
fn matmul_amx_vs_metal() {
    use ktir_cpu::metal_backend::NaxGemm;
    ...
    for (m, k, n, iters) in [(64usize, 2048usize, 8192usize, 20usize), (512, 4096, 4096, 20)] {
        ...
        let amx = median_secs(iters, || {
            std::hint::black_box(ktir_cpu::blas::sgemm_rowmajor(m, k, n, &a, &b));
        });
