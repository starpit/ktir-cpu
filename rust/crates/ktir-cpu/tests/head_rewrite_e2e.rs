...
fn load_bundle(dir: &Path) -> Bundle {
    ...
    for t in manifest["tensors"].as_array().unwrap() {
        ...
        shape.insert(id, (t["rows"].as_u64().unwrap() as usize, t["cols"].as_u64().unwrap() as usize, is_src));
        if is_src {
            sources.insert(id);
        }
    }
    ...
    for node in manifest["nodes"].as_array().unwrap() {
        ...
        let bindings = node["args"].as_array().unwrap().iter().map(|a| Binding {
    ...
    let spec = ProgramSpec { nodes, sources, results: HashSet::from([result_id]) };
    Bundle { module, spec, shape, result_id, mask_id }
}
...
fn head_rewrite_e2e_smollm2_prefill() {
    ...
    for (&id, &(rows, cols, is_src)) in &b.shape {
        if is_src && Some(id) != b.mask_id {
            owned.push((format!("t{id}"), Arg::Tensor {
    ...
    if let Some(m) = b.mask_id {
        ...
        owned.push((format!("t{m}"), Arg::Tensor { data: vec![0.0f32; rows * cols], shape: vec![rows, cols], dtype: DType::F16 }));
    }
    ...
    let run = |m: &IRModule| -> Vec<f32> {
        ...
        ktir_cpu::segmented::execute_segmented(m, &b.spec, &args, &[&result_key])
            .expect("execute_segmented")
            .get(&result_key).expect("result").data.clone()
    };
    ...
    let maxabs = |v: &[f32]| v.iter().zip(&golden).map(|(a, g)| (a - g).abs()).fold(0.0f32, f32::max);
    ...
    let budget = ktir_cpu::memory::lx_fusion_budget();
    let mut m2 = b.module.clone();
    let n_head = ktir_optimizer::head_rewrite::apply_head_rewrite(&mut m2, |sb| attention_needs_flash(sb, budget));
    let n_flash = ktir_optimizer::flash_attn::apply_flash_attention(&mut m2, |sb| attention_needs_flash(sb, budget));
    ...
    eprintln!(
        "\nsmollm2-135m-prefill e2e (best-of-3, release):\n  \
         ...
         e2e speedup   : {:.2}x",
        maxabs(&off_out), maxabs(&on_out), off_ms / on_ms
    );
    // Both must match golden (the passes are correctness-preserving).
    assert!(maxabs(&off_out) < 0.05, "WITHOUT-passes diverged from golden");
    assert!(maxabs(&on_out) < 0.05, "WITH-passes diverged from golden — the optimizer pass broke correctness e2e");
    assert!(n_head > 0, "head_rewrite did not fire on the prefill bundle");
}
