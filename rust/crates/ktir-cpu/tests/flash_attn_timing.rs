...
//! cost). Run:
//!   cargo test --release -p ktir-cpu --test flash_attn_timing -- --ignored \
//!     --nocapture --test-threads=1

use ktir_cpu::dtypes::DType;
use ktir_cpu::interpreter::{Arg, Output, execute_function};
use ktir_cpu::ir::{IRFunction, IRModule};
...
fn build_args(cap: i64) -> Vec<(&'static str, Arg)> {
    ...
    let mk = |name: &'static str, rows: usize, cols: usize, seed: usize| {
        (
            name,
            Arg::Tensor { data: arb(rows * cols, seed), shape: vec![rows, cols], dtype: f16 },
        )
    };
    vec![
        mk("q", m, qcols, 1),
        ...
        ("mask", Arg::Tensor { data: vec![0.0; cap], shape: vec![1, cap], dtype: f16 }),
        mk("kc", cap, kv_cols, 3),
        mk("kd", m, kv_cols, 4),
        mk("vc", cap, kv_cols, 5),
        mk("vd", m, kv_cols, 6),
...
fn flash_vs_untiled_wall_clock_sweep() {
    ...
    for &cap in CAPS {
        ...
        let blk = if fired { recover_blk(&fl_mod, &fl_name).unwrap_or(0) } else { 0 };
        ...
        match best_of_n(&un_mod, &un_name, &args) {
            Ok((un_ms, un_out)) => {
                ...
                for (k, o) in &un_out {
                    let r = fl_out.get(k).unwrap_or_else(|| panic!("flash missing output {k}"));
...
fn budget_fires_only_long_context_with_sensible_block() {
    ...
    assert!(!fired128, "cap=128 short context must NOT fire (identity no-op)");
