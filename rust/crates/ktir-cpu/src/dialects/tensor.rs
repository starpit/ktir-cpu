...
//
//! `tensor` dialect handlers — Rust port of `ktir_cpu/dialects/tensor_ops.py`.
//!
...
fn from_elements(
    ...
    for name in &op.operands {
        ...
        let s = match v {
            Value::Tile(t) => *t.data.first().unwrap_or(&0.0),
...
fn as_i64(v: &Value, name: &str) -> Result<i64, String> {
    match v {
        ...
        Value::Tile(t) => Ok(*t.data.first().unwrap_or(&0.0) as i64),
...
fn as_f32(v: &Value, name: &str) -> Result<f32, String> {
    match v {
        ...
        Value::Tile(t) => Ok(*t.data.first().unwrap_or(&0.0)),
