...
fn run_op_try(op: &Operation, seed: &[(&str, Value)]) -> Result<Value, String> {
    ...
    handler(op, &mut ctx, &env).map(|o| o.unwrap_or_else(|| panic!("op {:?} produced no value", op.op_type)))
}
...
#[test]
fn addf_scalars() {
    let r = run_op(&op("arith.addf", &["%a", "%b"]), &[("%a", sf(2.0)), ("%b", sf(3.0))]);
...
#[test]
fn divf_tile_scalar() {
    let r = run_op(&op("arith.divf", &["%a", "%b"]), &[("%a", f16_tile(&[4.0, 6.0, 8.0])), ("%b", sf(2.0))]);
    data_close(&as_tile(&r).data, &[2.0, 3.0, 4.0], 1e-2);
}

#[test]
fn divf_scalar_tile() {
    let r = run_op(&op("arith.divf", &["%a", "%b"]), &[("%a", sf(12.0)), ("%b", f16_tile(&[2.0, 3.0, 4.0]))]);
    data_close(&as_tile(&r).data, &[6.0, 4.0, 3.0], 1e-2);
}
...
fn minnumf_nan() {
    ...
    let r = run_op(
        ...
        &[("%a", f16_tile(&[f32::NAN, 3.0])), ("%b", f16_tile(&[2.0, f32::NAN]))],
    );
    let t = as_tile(&r);
...
fn addi_broadcast_tile() {
    let r = run_op(&op("arith.addi", &["%a", "%b"]), &[("%a", idx(10)), ("%b", f16_tile(&[1.0, 2.0, 3.0]))]);
...
fn muli_broadcast_tile() {
    let r = run_op(&op("arith.muli", &["%a", "%b"]), &[("%a", idx(2)), ("%b", f16_tile(&[1.0, 2.0, 3.0]))]);
    assert!(matches!(r, Value::Tile(_)));
}

#[test]
fn subi() {
    let r = run_op(&op("arith.subi", &["%a", "%b"]), &[("%a", idx(10)), ("%b", idx(3))]);
    assert_eq!(as_i64(&r), 7);
}

#[test]
fn remui() {
    let r = run_op(&op("arith.remui", &["%a", "%b"]), &[("%a", idx(10)), ("%b", idx(3))]);
    assert_eq!(as_i64(&r), 1);
}

#[test]
fn divsi_scalar() {
    let r = run_op(&op("arith.divsi", &["%a", "%b"]), &[("%a", idx(7)), ("%b", idx(2))]);
    assert_eq!(as_i64(&r), 3);
}

#[test]
fn divsi_truncates_toward_zero() {
    let r = run_op(&op("arith.divsi", &["%a", "%b"]), &[("%a", si(-7)), ("%b", si(2))]);
    assert_eq!(as_i64(&r), -3);
}

#[test]
fn remsi_scalar() {
    let r = run_op(&op("arith.remsi", &["%a", "%b"]), &[("%a", idx(7)), ("%b", idx(3))]);
    assert_eq!(as_i64(&r), 1);
}
...
fn remsi_negative() {
    ...
    let r = run_op(&op("arith.remsi", &["%a", "%b"]), &[("%a", si(-7)), ("%b", si(3))]);
    assert_eq!(as_i64(&r), -1);
}

#[test]
fn ceildivsi_scalar() {
    let r = run_op(&op("arith.ceildivsi", &["%a", "%b"]), &[("%a", idx(7)), ("%b", idx(2))]);
    assert_eq!(as_i64(&r), 4);
}

#[test]
fn ceildivui_scalar() {
    let r = run_op(&op("arith.ceildivui", &["%a", "%b"]), &[("%a", idx(7)), ("%b", idx(2))]);
    assert_eq!(as_i64(&r), 4);
}

#[test]
fn minsi_scalar() {
    let r = run_op(&op("arith.minsi", &["%a", "%b"]), &[("%a", idx(3)), ("%b", idx(7))]);
    assert_eq!(as_i64(&r), 3);
}

#[test]
fn minsi_negative() {
    let r = run_op(&op("arith.minsi", &["%a", "%b"]), &[("%a", si(-5)), ("%b", si(2))]);
    assert_eq!(as_i64(&r), -5);
}

#[test]
fn maxsi_scalar() {
    let r = run_op(&op("arith.maxsi", &["%a", "%b"]), &[("%a", idx(3)), ("%b", idx(7))]);
    assert_eq!(as_i64(&r), 7);
}
...
#[test]
fn minui_scalar() {
    let r = run_op(&op("arith.minui", &["%a", "%b"]), &[("%a", idx(3)), ("%b", idx(7))]);
    assert_eq!(as_i64(&r), 3);
}

#[test]
fn maxui_scalar() {
    let r = run_op(&op("arith.maxui", &["%a", "%b"]), &[("%a", idx(3)), ("%b", idx(7))]);
    assert_eq!(as_i64(&r), 7);
}

#[test]
fn floordivsi_scalar() {
    let r = run_op(&op("arith.floordivsi", &["%a", "%b"]), &[("%a", idx(7)), ("%b", idx(2))]);
    assert_eq!(as_i64(&r), 3);
}

#[test]
fn andi_scalar() {
    let r = run_op(&op("arith.andi", &["%a", "%b"]), &[("%a", idx(0b1010)), ("%b", idx(0b1100))]);
    assert_eq!(as_i64(&r), 0b1000);
}

#[test]
fn ori_scalar() {
    let r = run_op(&op("arith.ori", &["%a", "%b"]), &[("%a", idx(0b1010)), ("%b", idx(0b1100))]);
    assert_eq!(as_i64(&r), 0b1110);
}

#[test]
fn xori_scalar() {
    let r = run_op(&op("arith.xori", &["%a", "%b"]), &[("%a", idx(0b1010)), ("%b", idx(0b1100))]);
    assert_eq!(as_i64(&r), 0b0110);
}

#[test]
fn shli_scalar() {
    let r = run_op(&op("arith.shli", &["%a", "%b"]), &[("%a", idx(1)), ("%b", idx(3))]);
    assert_eq!(as_i64(&r), 8);
}

#[test]
fn shrsi_scalar() {
    let r = run_op(&op("arith.shrsi", &["%a", "%b"]), &[("%a", idx(8)), ("%b", idx(2))]);
    assert_eq!(as_i64(&r), 2);
}

#[test]
fn shrui_scalar() {
    let r = run_op(&op("arith.shrui", &["%a", "%b"]), &[("%a", idx(8)), ("%b", idx(2))]);
    assert_eq!(as_i64(&r), 2);
}
...
#[test]
fn remf_scalars() {
    let r = run_op(&op("arith.remf", &["%a", "%b"]), &[("%a", sf(5.0)), ("%b", sf(3.0))]);
...
#[test]
fn cmpf_olt_scalar() {
    let r = run_op(&cmpf_op("olt", &["%a", "%b"]), &[("%a", sf(1.0)), ("%b", sf(2.0))]);
    assert!(as_bool(&r));
}

#[test]
fn cmpf_ogt_scalar() {
    let r = run_op(&cmpf_op("ogt", &["%a", "%b"]), &[("%a", sf(3.0)), ("%b", sf(2.0))]);
    assert!(as_bool(&r));
}
...
#[test]
fn extui_tile() {
    let r = run_op(&op("arith.extui", &["%a"]), &[("%a", tile_with(&[1.0, 2.0, 3.0], DType::I32, &[3]))]);
    let t = as_tile(&r);
...
fn convertf_f16_to_f32() {
    let r = run_op(&op("arith.convertf", &["%a"]), &[("%a", f16_tile(&[1.0, 2.0]))]);
    assert_eq!(as_tile(&r).dtype, DType::F32);
}

#[test]
fn convertf_f32_to_f16() {
    let r = run_op(&op("arith.convertf", &["%a"]), &[("%a", tile_with(&[1.0, 2.0], DType::F32, &[2]))]);
...
#[test]
fn cmpi_scalar() {
    let r = run_op(&cmpi_op("slt", &["%a", "%b"]), &[("%a", idx(1)), ("%b", idx(2))]);
    assert!(as_bool(&r));
}
...
#[test]
fn cmpi_ult() {
    let r = run_op(&cmpi_op("ult", &["%a", "%b"]), &[("%a", idx(1)), ("%b", idx(2))]);
    assert!(as_bool(&r));
}
...
#[test]
fn select_scalar() {
    let r = run_op(&op("arith.select", &["%cond", "%t", "%f"]), &[("%cond", Value::Scalar(Scalar::Bool(true))), ("%t", idx(10)), ("%f", idx(20))]);
...
fn math_exp_tile() {
    ...
    data_close(&as_tile(&r).data, &[1.0, std::f32::consts::E], 1e-2);
}
...
#[test]
fn math_sqrt_tile() {
    let r = run_op(&op("math.sqrt", &["%x"]), &[("%x", f16_tile(&[4.0, 9.0, 16.0]))]);
    data_close(&as_tile(&r).data, &[2.0, 3.0, 4.0], 1e-2);
}
...
#[test]
fn math_log_tile() {
    let r = run_op(&op("math.log", &["%x"]), &[("%x", f16_tile(&[1.0, 2.0, 4.0]))]);
    data_close(&as_tile(&r).data, &[0.0, 2.0f32.ln(), 4.0f32.ln()], 1e-2);
}
...
#[test]
fn math_rsqrt_tile() {
    let r = run_op(&op("math.rsqrt", &["%x"]), &[("%x", f16_tile(&[1.0, 4.0, 16.0]))]);
    data_close(&as_tile(&r).data, &[1.0, 0.5, 0.25], 1e-2);
}
...
#[test]
fn math_log2_tile() {
    let r = run_op(&op("math.log2", &["%x"]), &[("%x", f16_tile(&[1.0, 2.0, 8.0]))]);
    data_close(&as_tile(&r).data, &[0.0, 1.0, 3.0], 1e-2);
}
...
#[test]
fn math_log1p_tile() {
    let r = run_op(&op("math.log1p", &["%x"]), &[("%x", f16_tile(&[0.0, 1.0, 2.0]))]);
    data_close(&as_tile(&r).data, &[0.0, 1.0f32.ln_1p(), 2.0f32.ln_1p()], 1e-2);
}
...
#[test]
fn math_tanh_tile() {
    let r = run_op(&op("math.tanh", &["%x"]), &[("%x", f16_tile(&[0.0, 1.0, -1.0]))]);
    data_close(&as_tile(&r).data, &[0.0, 1.0f32.tanh(), (-1.0f32).tanh()], 1e-2);
}
...
#[test]
fn math_sin_tile() {
    let r = run_op(&op("math.sin", &["%x"]), &[("%x", f16_tile(&[0.0, 1.5708, 3.1416]))]);
    data_close(&as_tile(&r).data, &[0.0, 1.5708f32.sin(), 3.1416f32.sin()], 2e-2);
}
...
#[test]
fn math_cos_tile() {
    let r = run_op(&op("math.cos", &["%x"]), &[("%x", f16_tile(&[0.0, 1.5708, 3.1416]))]);
    data_close(&as_tile(&r).data, &[1.0, 1.5708f32.cos(), 3.1416f32.cos()], 2e-2);
}
...
#[test]
fn math_powf_tile() {
    let r = run_op(&op("math.powf", &["%a", "%b"]), &[("%a", f16_tile(&[2.0, 3.0, 4.0])), ("%b", f16_tile(&[2.0, 2.0, 0.5]))]);
    data_close(&as_tile(&r).data, &[4.0, 9.0, 2.0], 1e-2);
}
...
#[test]
fn math_erf_tile() {
    let r = run_op(&op("math.erf", &["%x"]), &[("%x", f16_tile(&[0.0, 1.0, -1.0]))]);
    data_close(&as_tile(&r).data, &[0.0, 0.8427, -0.8427], 1e-2);
}
...
#[test]
fn math_absi_scalar() {
    let r = run_op(&op("math.absi", &["%x"]), &[("%x", Value::Scalar(Scalar::I32(-7)))]);
    assert_eq!(as_i64(&r), 7);
}

#[test]
fn math_powf_scalar() {
    let r = run_op(&op("math.powf", &["%a", "%b"]), &[("%a", sf(2.0)), ("%b", sf(3.0))]);
    assert_eq!(as_f32(&r), 8.0);
}

#[test]
fn math_fma_scalar() {
    let r = run_op(&op("math.fma", &["%a", "%b", "%c"]), &[("%a", sf(3.0)), ("%b", sf(4.0)), ("%c", sf(1.0))]);
...
fn reduce_along_dim() {
    ...
    let r = run_op_execute(
        &o,
        &[("%x", tile_with(&[1.0, 2.0, 3.0, 4.0], DType::F16, &[1, 4])), ("%init", tile_with(&[0.0], DType::F16, &[1]))],
    );
    let val = match &r {
        Value::Tile(t) => t.data[0],
...
#[test]
#[ignore = "multi-axis reduce not supported (Python xfail, issue #85): parser keeps dims[0] only"]
fn reduce_multi_axis() {
    // dimensions=[0,1] on a 2x3 should reduce both axes -> 15. Not supported.
}

#[test]
#[ignore = "outs init not folded into reduction (Python xfail, issue #85)"]
fn reduce_folds_outs_init() {
    // sum([1,2,3,4]) with outs init 100 should be 110. Not folded.
}

#[test]
fn fill() {
    let r = run_op(&op("linalg.fill", &["%val", "%out"]), &[("%val", sf(3.0)), ("%out", tile_with(&[0.0; 4], DType::F16, &[4]))]);
    let t = as_tile(&r);
    assert_eq!(t.shape, vec![4]);
    assert!(t.data.iter().all(|&x| x == 3.0));
}

#[test]
fn broadcast() {
    let o = op("linalg.broadcast", &["%inp", "%out"]).with_attr("dimensions", Attr::IntList(vec![0]));
    let r = run_op(&o, &[("%inp", tile_with(&[1.0, 2.0, 3.0, 4.0], DType::F16, &[4])), ("%out", tile_with(&[0.0; 8], DType::F16, &[2, 4]))]);
    let t = as_tile(&r);
    assert_eq!(t.shape, vec![2, 4]);
    assert_eq!(&t.data[0..4], &[1.0, 2.0, 3.0, 4.0]);
    assert_eq!(&t.data[4..8], &[1.0, 2.0, 3.0, 4.0]);
}

#[test]
fn matmul() {
    // identity @ B == B.
    let r = run_op(&op("linalg.matmul", &["%a", "%b"]), &[("%a", tile_with(&[1.0, 0.0, 0.0, 1.0], DType::F16, &[2, 2])), ("%b", tile_with(&[1.0, 2.0, 3.0, 4.0], DType::F16, &[2, 2]))]);
...
fn batch_matmul() {
    ...
    let r = run_op(&op("linalg.batch_matmul", &["%a", "%b"]), &[("%a", tile_with(&eye, DType::F16, &[3, 2, 2])), ("%b", tile_with(&bdata, DType::F16, &[3, 2, 2]))]);
    let t = as_tile(&r);
    assert_eq!(t.shape, vec![3, 2, 2]);
    data_close(&t.data, &bdata, 1e-2);
}

#[test]
fn generic_reads_outs_arg() {
    // linalg.generic body reads outs bb0 arg: outs (1,2) + ins (10,20) = (11,22).
    let bb0 = Operation::new(None, "region.bb0_args", &[]).with_attr("names", Attr::StrList(vec!["%in_arg".into(), "%out_arg".into()]));
    ...
    let mut o = op("linalg.generic", &["%ins", "%outs"])
        ...
        .with_attr("bb0_names", Attr::StrList(vec!["%in_arg".into(), "%out_arg".into()]));
    o.regions = vec![vec![bb0, add, yld]];
    let r = run_op_execute(&o, &[("%ins", f16_tile(&[10.0, 20.0])), ("%outs", f16_tile(&[1.0, 2.0]))]);
    data_close(&as_tile(&r).data, &[11.0, 22.0], 1e-2);
}
...
#[test]
fn tensor_empty() {
    let o = Operation::new(Some("%r"), "tensor.empty", &[]).with_attr("shape", Attr::IntList(vec![2, 4])).with_attr("dtype", Attr::Str("f16".into()));
    let r = run_op(&o, &[]);
    let t = as_tile(&r);
    assert_eq!(t.shape, vec![2, 4]);
}
...
#[test]
fn tensor_splat() {
    let o = op("tensor.splat", &["%val"]).with_attr("shape", Attr::IntList(vec![4])).with_attr("dtype", Attr::Str("f16".into()));
...
fn tensor_extract() {
    ...
    let r = run_op(&op("tensor.extract", &["%t", "%i", "%j"]), &[("%t", tile_with(&[1.0, 2.0, 3.0, 4.0], DType::F16, &[2, 2])), ("%i", idx(1)), ("%j", idx(0))]);
    close(as_f32(&r), 3.0, 1e-2);
}
...
fn tensor_expand_shape() {
    ...
    let r = run_op(&o, &[("%t", tile_with(&[1.0, 2.0, 3.0, 4.0], DType::F16, &[4]))]);
...
#[test]
fn tensor_reshape() {
    let o = op("tensor.reshape", &["%t", "%s"]).with_attr("target_shape", Attr::IntList(vec![2, 4])).with_attr("dtype", Attr::Str("f16".into()));
...
fn tensor_reshape_non_square_target() {
    let o = op("tensor.reshape", &["%t", "%s"]).with_attr("target_shape", Attr::IntList(vec![3, 4])).with_attr("dtype", Attr::Str("f16".into()));
...
fn tensor_reshape_size_mismatch_raises() {
    ...
    let o = op("tensor.reshape", &["%t", "%s"]).with_attr("target_shape", Attr::IntList(vec![3, 3])).with_attr("dtype", Attr::Str("f16".into()));
    let data: Vec<f32> = (0..7).map(|x| x as f32).collect();
    let err = run_op_try(&o, &[("%t", tile_with(&data, DType::F16, &[7])), ("%s", tile_with(&[3.0, 3.0], DType::I32, &[2]))]).unwrap_err();
    assert!(err.contains("cannot reshape"), "unexpected error: {err}");
}

#[test]
fn tensor_reshape_to_3d() {
    let o = op("tensor.reshape", &["%t", "%s"]).with_attr("target_shape", Attr::IntList(vec![2, 3, 4])).with_attr("dtype", Attr::Str("f16".into()));
...
fn tensor_from_elements() {
    let o = op("tensor.from_elements", &["%a", "%b"]).with_attr("shape", Attr::IntList(vec![2])).with_attr("dtype", Attr::Str("index".into()));
...
fn tensor_from_elements_n1() {
    let o = op("tensor.from_elements", &["%a"]).with_attr("shape", Attr::IntList(vec![1])).with_attr("dtype", Attr::Str("index".into()));
...
fn generate_1d() {
    ...
    let bb0 = Operation::new(None, "region.bb0_args", &[]).with_attr("names", Attr::StrList(vec!["%i".into()]));
...
fn generate_2d() {
    ...
    let bb0 = Operation::new(None, "region.bb0_args", &[]).with_attr("names", Attr::StrList(vec!["%i".into(), "%j".into()]));
    let cmp = Operation::new(Some("%cmp"), "arith.cmpi", &["%i", "%j"]).with_attr("predicate", Attr::Str("sge".into()));
...
fn scf_yield() {
    ...
    let r = run_op(&op_noresult("scf.yield", &["%a", "%b"]), &[("%a", idx(5)), ("%b", idx(6))]);
    match r {
        Value::Tuple(vals) => {
...
fn if_then_else_yield_result() {
    ...
    let r = run_op_execute(&o, &[("%cond", Value::Scalar(Scalar::Bool(true))), ("%val", idx(42))]);
    assert_eq!(as_i64(&r), 42);
}
...
fn get_compute_tile_id_single() {
    ...
    let r = dispatch.handler("ktdp.get_compute_tile_id").unwrap()(&o, &mut ctx, &env).unwrap().unwrap();
    assert_eq!(as_i64(&r), 3);
}
...
fn get_compute_tile_id_multi() {
    ...
    let o = Operation::new(Some("%x"), "ktdp.get_compute_tile_id", &[]).with_attr("num_results", Attr::Int(2));
    let r = dispatch.handler("ktdp.get_compute_tile_id").unwrap()(&o, &mut ctx, &env).unwrap().unwrap();
    match r {
        Value::Tuple(vals) => {
            assert_eq!(vals.len(), 2);
...
fn construct_memory_view() {
    ...
    match r {
        Value::MemRef(m) => {
            ...
            // byte_address == stick index * STICK_BYTES; base stick == ptr.
            assert_eq!(m.byte_address(), ptr * STICK_BYTES);
        }
...
fn load_store_roundtrip() {
    ...
    use ktir_cpu::affine::AffineMap;
    use ktir_cpu::interpreter::execute_ops;
    ...
    let bytes = ktir_cpu::codec::encode(&data, DType::F16);
    ctx.hbm.borrow_mut().write_bytes(stick * STICK_BYTES, &bytes);
    ...
    ctx.set_value("%tile", Value::Tile(Tile::compute(doubled.clone(), DType::F16, vec![n])));
    execute_ops(&[access("%acc2"), Operation::new(None, "ktdp.store", &["%tile", "%acc2"])], &mut ctx, &env).unwrap();
    let raw = ctx.hbm.borrow().read_bytes(stick * STICK_BYTES, n * 2);
    let got = ktir_cpu::codec::decode(&raw, n, DType::F16);
