...
use ktir_cpu::ir::{Attr, Operation, Value};
use ktir_cpu::memory::STICK_BYTES;
use ktir_cpu::memref::{MemRef, MemorySpace, ParentRef, TileRef};
use ktir_cpu::ops_memory::{load_data, store_data};
use ktir_cpu::parser_ast::{parse_affine_map, parse_affine_set};
...
/// Build an HBM `MemRef` (stick-indexed `base_ptr`), mirroring the Python
/// `MemRef(base_ptr=ptr, shape=..., strides=..., memory_space="HBM", dtype="f16")`.
...
fn alloc_f16(ctx: &mut ktir_cpu::context::CoreContext, data: &[f32], shape: &[usize]) -> i64 {
...
fn affine_attrs_preserved() {
    ...
    let module = ktir_cpu::parser::parse_module(&src).expect("parse module");
